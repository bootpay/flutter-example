...
class CommercePayment extends StatefulWidget {
  const CommercePayment({Key? key}) : super(key: key);

  @override
  State<CommercePayment> createState() => _CommercePaymentState();
}

class _CommercePaymentState extends State<CommercePayment> {
  // 화면 내 환경 토글 UI용 — BootpayEnvConfig.devClientKey/prodClientKey 에서 읽음
  // (.env 미설정 시 BootpayEnvConfig 내부 production/development fallback 사용)
  late final Map<String, Map<String, dynamic>> envConfig = {
    'development': {
      'client_key': BootpayEnvConfig.devClientKey,
      'plans': {
        'starter': {
    ...
    'production': {
      'client_key': BootpayEnvConfig.prodClientKey,
      'plans': {
        'starter': {
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor:
                          isPopular ? primaryColor : const Color(0xFFF1F5F9),
                      foregroundColor:
                          isPopular ? Colors.white : const Color(0xFF475569),
  ...
  void _startPayment() {
    ...
    final orderId = payload.requestId ??
    ...
    BootpayCommerce.requestCheckout(
      ...
      onDone: (data) => _showPaymentResult(data),
      onError: (data) => _showPaymentResult(data),
      onCancel: (data) => _showPaymentResult(data),
      onIssued: (data) => _showPaymentResult(data),
      onClose: () {},
    );
  }
  ...
  void _showPaymentResult(Map<String, dynamic> data) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => _CommerceResultPage(data: data)),
    );
  }
}
                          ...
                          if (data['metadata'] != null) ...[
                            if ((data['metadata'] as Map)['plan_key'] != null)
                              _buildInfoRow(
                                '플랜',
                                _capitalize(
                                  (data['metadata'] as Map)['plan_key']
                                      .toString(),
                                ),
                              ),
                            if ((data['metadata'] as Map)['billing_type'] !=
                                null)
                              _buildInfoRow(
                                '결제 주기',
                                (data['metadata'] as Map)['billing_type']
                                    .toString(),
                              ),
                          ],
                          if (event.isNotEmpty) _buildInfoRow('이벤트', event),
                        ],
                      ),
                    ),
                  ],
